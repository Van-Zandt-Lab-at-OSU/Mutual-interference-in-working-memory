...
#####################
#####################
# Model for the numerical version
...
functions {
  ...
  real mc_lpdf(vector rt,int md,row_vector prevector,matrix randommatrix,
       int[] loc,int[] accuracy,int[] ltype,int[] swit_rep,real mu1,real mu2,real mu3,real tau,
...
model {
   ...
   for(n in 1:Ngroup){
      for(l in 1:2){
         ...
         kappa_c[n,l] ~ normal(kappa_u_0[l],0.2);
         v_r_c[n,l] ~ normal(v_r_0[l],0.2);
   ...
   for(n in 1:Nsub){
      cmat[n,1,1] = compute_matrix(C[n,1],sigma[n,1],3,9);
      cmat[n,1,2] = compute_matrix(C[n,2],sigma[n,2],3,9);
      cmat[n,2,1] = compute_matrix(C[n,1],sigma[n,1],5,9);
      cmat[n,2,2] = compute_matrix(C[n,2],sigma[n,2],5,9);
   }
   ...
   for(n in 1:N){
      M = task[n];
      K = id[n];
      target += mc_lpdf(rt[n] | md[n],prevector,randommatrix,loc[n],accuracy[n],ltype[n],swit_rep[n],
                   mu1[K],mu2[K],mu3[K],tau[K],v_u[K,M],b[K,M],v_r[K,M],kappa[K,M],
                   sdd_mu2[K],a_u[K,M],a_r[K,M],q[K,M],cmat[K,md_id[n],M]);
   }
}
"
...
dat$tau_mean = rep(0.15,dat$Nsub)
